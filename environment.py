...
class Environment(testmod.Environment):
    ...
    def __init__(self, opts):
        ...
        self.c2 = nfs4client.NFS4Client(opts.server, opts.port, opts.minorversion)
        self.c3 = nfs4client.NFS4Client(opts.server, opts.port, opts.minorversion)
        self.c4 = nfs4client.NFS4Client(opts.server, opts.port, opts.minorversion)
        ...
        self.c2.set_cred(self.cred1)
        self.c3.set_cred(self.cred1)
        self.c4.set_cred(self.cred1)
        self.cred2 = AuthSys().init_cred(uid=1111, gid=37, name="shampoo")

        opts.home = opts.path + ['tmp']
        self.c1.homedir = opts.home
        self.c2.homedir = opts.home
        self.c3.homedir = opts.home
        self.c4.homedir = opts.home
    ...
    def finish(self):
        ...
        if self.opts.nocleanup:
            return
        sess = self.c1.new_client_session("Environment.finish_%i" % self.timestamp)
        clean_dir(sess, self.opts.home)
        sess.c.null()
    ...
    def shutDown(self):
        ...
        client_index = 0
        clients = {}
        for sessid in self.c1.sessions:
            session = self.c1.sessions[sessid]
            if session is None:
                continue
            clients[client_index] = session.client
            client_index += 1
            res = self.c1.compound([op.destroy_session(sessid)])
            if res.status != NFS4_OK:
             log.info("Could not clean up session: status %s" % nfsstat4[res.status])
             session.client._del_session(session)
        for client_index in clients:
            client = clients[client_index]
            if client == self.c1:
                continue
            res = self.c1.compound([op.destroy_clientid(client.clientid)])
            if res.status != NFS4_OK:
             log.info("Could not clean up client: status %s" % nfsstat4[res.status])
        log.debug("done post-test")

    def sleep(self, sec, msg=''):
        """Sleep for given seconds"""
        log.info("Sleeping for %g seconds: %s" % (sec, msg))
        time.sleep(sec)
        log.info("Woke up")
...
def create_file_return(sess, owner, path=None, attrs={FATTR4_MODE: 0644},
                access=OPEN4_SHARE_ACCESS_READ,
                deny=OPEN4_SHARE_DENY_NONE,
                mode=GUARDED4, verifier=None, want_deleg=True,
                # Setting the following should induce server errors
                seqid=1, clientid=1):
    ...
    return open_create_file(sess, owner, path, attrs, access, deny, mode,
                            verifier, claim_type, want_deleg, deleg_type,
                            open_create_file, seqid, clientid)
